...
use crate::msg::{LevelResponse, InstantiateMsg, ExecuteMsg, QueryMsg, ScoreResponse, MigrateMsg};
...
pub fn instantiate(
  ...
  let state = State {
    level: msg.level,
    owner: info.sender.clone(), // sender addressのcopyが欲しいだけだからcloneを使ってる
    scores: vec![],
  };
  ...
  Ok(Response::new()
    ...
    .add_attribute("level", msg.level.to_string())
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
  match msg {
    QueryMsg::GetLevel {} => to_binary(&query_level(deps)?),
    QueryMsg::GetScores {} => to_binary(&query_scores(deps)?),
  }
}

fn query_level(deps: Deps) -> StdResult<LevelResponse> {
  let state = STORAGE.load(deps.storage)?;
  Ok(LevelResponse { level: state.level }) // msg.rsにCountResponseの定義がある
}
